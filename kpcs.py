...
if uploaded_file is not None:
    ...
    # --- SỬA ĐỔI: TẠO 2 CỘT CHO 2 NÚT BẤM RIÊNG BIỆT ---
    ...
    with col1:
        if st.button("🚀 Tạo 7 Báo cáo Tổng hợp"):
            ...
            st.success("🎉 Đã tạo xong file Excel chứa 7 báo cáo!")
            st.download_button(label="📥 Tải xuống File 7 Báo cáo", data=excel_data, file_name=f"Tong_hop_7_Bao_cao_KPCS_Q{input_quarter}_{input_year}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    with col2:
        # --- CHỨC NĂNG MỚI: THỐNG KÊ CHI TIẾT KIẾN NGHỊ ---
        if st.button("📋 Thống kê chi tiết Kiến nghị"):
            with st.spinner("⏳ Đang lọc danh sách kiến nghị..."):
                q_start_date = pd.to_datetime(f'{input_year}-{(input_quarter-1)*3 + 1}-01')
                q_end_date = q_start_date + pd.offsets.QuarterEnd(0)
                
                # Lọc các kiến nghị được ban hành trong quý
                df_filtered = df_raw[(df_raw['Ngày, tháng, năm ban hành (mm/dd/yyyy)'] >= q_start_date) & (df_raw['Ngày, tháng, năm ban hành (mm/dd/yyyy)'] <= q_end_date)].copy()
                
                # Tạo file excel trong bộ nhớ cho danh sách này
                output_stream_detail = BytesIO()
                with pd.ExcelWriter(output_stream_detail, engine='xlsxwriter') as writer:
                    df_filtered.to_excel(writer, sheet_name=f"Chi_tiet_KN_Q{input_quarter}_{input_year}", index=False)
                excel_data_detail = output_stream_detail.getvalue()

            st.success(f"🔍 Tìm thấy {len(df_filtered)} kiến nghị trong Quý {input_quarter}/{input_year}.")
            st.write("Dưới đây là danh sách chi tiết:")
            st.dataframe(df_filtered)
            
            st.download_button(label="📥 Tải xuống danh sách chi tiết", data=excel_data_detail, file_name=f"Chi_tiet_Kien_nghi_Q{input_quarter}_{input_year}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
