...
# --- Hàm cho 7 báo cáo Excel ---
def calculate_summary_metrics_for_excel(dataframe, groupby_cols, year_start_date, quarter_start_date, quarter_end_date):
    if not isinstance(groupby_cols, list):
        raise TypeError("groupby_cols phải là một danh sách (list)")
    def agg(data_filtered, cols):
...
# --- Hàm cho chức năng "Thống kê kiến nghị" mới ---
def calculate_metrics_for_audit_report(df_group, group_by_col=None):
    ...
    if group_by_col is None:
        ...
        summary = df_group.groupby(group_by_col).agg(**{'Thời gian phát hành báo cáo': (ISSUE_DATE_COL, 'first'), 'Tổng kiến nghị': (group_by_col, 'size'), 'Đã khắc phục': (FIXED_COL, lambda x: (x == 'X').sum())})
    if group_by_col:
    ...
    for col in summary.select_dtypes(include=np.number).columns: summary[col] = summary[col].astype(int)
    return summary.reset_index()

def generate_quarterly_audit_report(df, year, quarter):
    q_start_date = pd.to_datetime(f'{year}-{(quarter-1)*3 + 1}-01')
    q_end_date = q_start_date + pd.offsets.QuarterEnd(0)
    df_quarter = df[(df['Ngày, tháng, năm ban hành (mm/dd/yyyy)'] >= q_start_date) & (df['Ngày, tháng, năm ban hành (mm/dd/yyyy)'] <= q_end_date)].copy()
    ...
    for group_key, group_display_name in group_order_and_names.items():
        ...
        header_row = calculate_metrics_for_audit_report(df_main_group, group_by_col=None)
        ...
        detail_rows = calculate_metrics_for_audit_report(df_main_group, group_by_col='Tên Đoàn kiểm toán')
    ...
    grand_total_row = calculate_metrics_for_audit_report(df_quarter, group_by_col=None)
...
if uploaded_file is not None:
    ...
    # --- TẠO 2 NÚT BẤM CHỨC NĂNG ---
    col1, col2 = st.columns(2)

    with col1:
        if st.button("🚀 Tạo 7 Báo cáo Excel"):
            # (Phần code này giữ nguyên logic tạo file Excel của bạn)
            # Bạn có thể paste lại code tạo 7 báo cáo vào đây nếu cần
            st.info("Chức năng tạo 7 báo cáo Excel đang được xây dựng.")

    # <<< NÚT CHỨC NĂNG MỚI >>>
    with col2:
        if st.button("📈 Thống kê Kiến nghị trong Quý"):
            with st.spinner("⏳ Đang thống kê kiến nghị..."):
                df = df_raw.copy()
                # Đảm bảo các cột cần thiết tồn tại
                required_cols = ['Đoàn KT/GSTX', 'Tên Đoàn kiểm toán', 'Ngày, tháng, năm ban hành (mm/dd/yyyy)',
                ...
                if not all(col in df.columns for col in required_cols):
                    st.error(f"Lỗi: File tải lên thiếu một trong các cột cần thiết: {required_cols}")
                else:
                    audit_summary_df = generate_quarterly_audit_report(df, input_year, input_quarter)
                    st.success("✅ Thống kê hoàn tất!")
                    st.markdown("### Bảng Kết quả kiểm toán trong Quý")
